#include <iostream>
#include <string>
#include <vector>
using namespace std;

bool match_pattern(const string& line, const string& pattern, int line_pos, int pattern_pos) {
    // Base case: if pattern is exhausted, we found a match
    if (pattern_pos == pattern.length()) return true;
    
    // Base case: if line is exhausted but pattern isn't, no match
    if (line_pos == line.length()) return false;
    
    // Handle the current character
    char p = pattern[pattern_pos];
    
    // Case 1: If next character is '*'
    if (pattern_pos + 1 < pattern.length() && pattern[pattern_pos + 1] == '*') {
        // Try matching current character 0 or more times
        // Either skip the '*' pattern (0 occurrences)
        if (match_pattern(line, pattern, line_pos, pattern_pos + 2)) return true;
        
        // Or match current character and try to match rest
        if (p == '.' || p == line[line_pos]) {
            if (match_pattern(line, pattern, line_pos + 1, pattern_pos)) return true;
        }
        return false;
    }
    
    // Case 2: Normal character or '.'
    if (p == '.' || p == line[line_pos]) {
        return match_pattern(line, pattern, line_pos + 1, pattern_pos + 1);
    }
    
    return false;
}

bool find_pattern(const string& line, const string& pattern) {
    // Try matching pattern starting from each position in line
    for (int i = 0; i <= line.length(); i++) {
        if (match_pattern(line, pattern, i, 0)) return true;
    }
    return false;
}

int main() {
    vector<string> lines, patterns;
    string line;
    
    // Read input lines
    while (getline(cin, line)) {
        if (line == "---") break;
        lines.push_back(line);
    }
    
    // Read patterns
    while (getline(cin, line)) {
        if (line == "---") break;
        patterns.push_back(line);
    }
    
    // Process each pattern
    for (const string& pattern : patterns) {
        bool found = false;
        vector<int> matching_lines;
        
        // Check each line for the current pattern
        for (int i = 0; i < lines.size(); i++) {
            if (find_pattern(lines[i], pattern)) {
                found = true;
                matching_lines.push_back(i + 1);  // +1 for 1-based line numbers
            }
        }
        
        // Print results
        if (!found) {
            cout << "-";
        } else {
            for (int i = 0; i < matching_lines.size(); i++) {
                if (i > 0) cout << " ";
                cout << matching_lines[i];
            }
        }
        cout << endl;
    }
    
    return 0;
}