...
void dijkstra(vector<vector<char> > &graph,int n,int m){
    int V = n*m;
    ...
    while(!pq.empty()){
        ...
        for(int i = 0;i < 4;i++){
            ...
            if(sr >= 0 && sr < n && sc >= 0 && sc < m){
                if(SSet[sr][sc] == false && graph[ur][uc] 
                && key[ur][uc] != INT_MAX && key[ur][uc] + graph[sr][sc] < key[sr][sc]){
                    key[sr][sc] = key[ur][uc] + graph[sr][sc];
